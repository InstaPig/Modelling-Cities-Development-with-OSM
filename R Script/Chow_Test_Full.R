#######################################
## Chow Test for Linear Relationship ##
#######################################
...
m2 <- lm(self_reinforcement ~ dist_to_centre, data = stg_data_1)
m3 <- lm(log_update_rate ~ dist_to_centre, data = stg_data_1)
m4 <- lm(self_reinforcement ~ dist_to_centre, data = stg_data_1)
...
x1 <- stg_data_1$dist_to_centre
x2 <- stg_data_2$dist_to_centre
Geographic <- chow.test(y1,x1,y2,x2)
...
x2 <- as.matrix(stg_data_2[c("median_house_price",
                             "number_of_jobs_in_area",
                             "median_household_income_estimate")])
Economic <- chow.test(y1,x1,y2,x2)

x1 <- as.matrix(stg_data_1[c("bame_rate", "population",
                             "mean_age", "median_age", "employment_rate",
                             "imd_score")])
x2 <- as.matrix(stg_data_2[c("bame_rate", "population",
                             "mean_age", "median_age", "employment_rate",
                             "imd_score")])
Socio_demographic <- chow.test(y1,x1,y2,x2)
...
All_groups <- chow.test(y1,x1,y2,x2)


#chow_table <- as.data.frame(rbind(Geographic,Economic,Socio_demographic,All_groups))
chow_table_2 <- as.data.frame(rbind(Geographic,Economic,Socio_demographic,All_groups))
